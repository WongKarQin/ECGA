from de_algorithm_RIECM import *
...
#'BNB',
#'MNB', 'CONB', 'GNB',
#'CART', 'SVM', 'MLP', 'CANB', 'LR', 'KNN', 'SGD', 'RF'
#str_list = ['MLP','CART', 'SVM', 'CANB', 'LR', 'KNN', 'SGD', 'RF']
str_list = ['CART']
# load data set
...
x_max = 10
max_vel = 0.5
opts = {'classifer': '', 'dim': dim, 'N': number_particles, 'fold': fold, 'T': iter_num, 'x_max': x_max,
        'max_vel': max_vel}
for classifer_name in str_list:
	opts['classifer'] = classifer_name
	with open('../result/calculation_statistic.txt', 'a') as f:  # 设置文件对象
		f.write('\n' + 'DE_iter_100_size_100_ECDE_' + classifer_name + ',')  # 将字符串写入文件中
		f.close()
	#with open('../result/calculation_F1.txt', 'a') as f:  # 设置文件对象
	#	f.write('\n' + 'DE_iter_100_size_100_RIECM_' + classifer_name + ',')  # 将字符串写入文件中
	#	f.close()
	...
	with open('../result/calculation_num_Feature.txt', 'a') as f:  # 设置文件对象
		f.write('\n' + 'DE_iter_100_size_100_ECDE_' + classifer_name + ',')  # 将字符串写入文件中
		f.close()
	with open('../result/calculation_feature_schedule.txt', 'a') as f:  # 设置文件对象
		f.write('\n' + 'DE_iter_100_size_100_ECDE_' + classifer_name + ',')  # 将字符串写入文件中
		f.close()
	for i in range(0, 10):
		time_1 = time.time()
